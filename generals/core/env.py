...
Multiplayer: the environment supports an arbitrary (but JIT-static) number of
players, and an optional team assignment. Free-for-all is the default
(teams = [0, 1, ..., N-1]); team modes like 2v2 use teams = [0, 0, 1, 1].
...
class TimeStep(NamedTuple):
    ...
    Attributes:
        observation: Observations for all players, stacked along axis 0 (shape (N, ...)).
        reward: Array of shape (N,) with rewards per player.
        terminated: Boolean scalar, True if game ended (winner decided).
...
class GeneralsEnv:
    ...
    Multiplayer:
        - num_players: number of players (default 2). JIT-static.
        - teams: (N,) int32 array assigning a team_id to each player. Defaults to
          arange(num_players) (free-for-all). For 2v2 pass jnp.array([0, 0, 1, 1]).
    """
    def __init__(
        self,
    ...
    def _make_single_state_fixed(self, key: jnp.ndarray, h: int, w: int) -> GameState:
        ...
        return create_initial_state(grid.astype(jnp.int32), self.teams)
    ...
    def step(
        ...
        Args:
            ...
            actions: Array of shape (N, 5), one [pass, row, col, direction, split]
                per player.
        ...
        # +1 for winning team, -1 for losing teams, 0 if ongoing
        team_won = state.teams == info.winner  # (N,)
        rewards = jnp.where(info.winner >= 0, jnp.where(team_won, 1.0, -1.0), 0.0)
        ...
        per_player_obs = [get_obs(final_state, i) for i in range(self.num_players)]
        observation = jax.tree.map(lambda *xs: jnp.stack(xs, axis=0), *per_player_obs)
