...
def _apply_move(
    ...
    ownership_attack = lax.cond(
        attacker_wins, lambda o: o.at[player_idx, di, dj].set(True), lambda o: o, ownership_attack
    )
...
def global_update(state: GameState) -> GameState:
    ...
    armies = lax.cond(
        ...
        lambda a: a
        + structure_mask * state.ownership[0].astype(jnp.int32)
...
def get_observation(state: GameState, player_idx: int) -> Observation:
    ...
    Returns:
        Observation named tuple with observation arrays
    """
