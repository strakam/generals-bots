"""
Minimal JAX PPO for Generals.io - fully vectorized and fast.

Based on gpu_perf_2.py with PPO training added.
Everything is JIT-compiled and vmapped for maximum speed.

Modified to:
- Train for 200 iterations
- Save model after training
- Visualize policy on 20 episodes with rendering
"""
...
class TinyNetwork(eqx.Module):
    ...
    def __call__(self, obs, mask, key, action=None):
        ...
        grid_size = obs.shape[-1]  # Infer grid size from observation
...
@jax.jit
def rollout_step(states, network, grids, key):
    """Single vectorized rollout step."""
    ...
    new_obs_p0 = jax.vmap(lambda s: game_jax.get_observation(s, 0))(new_states)
    
    # Rewards - composite reward function (prior_obs, action, new_obs)
    rewards = jax.vmap(composite_reward_fn)(obs_p0, actions_p0, new_obs_p0)
    
    # Reset done envs
    fresh = jax.vmap(game_jax.create_initial_state)(grids)
    states = jax.tree.map(
        lambda f, c: jnp.where(dones.reshape(num_envs, *([1] * (f.ndim - 1))), f, c),
        fresh, new_states
    )
    
    return states, (obs_arr, masks, actions_p0, logprobs, values, rewards, dones, infos), key
...
def visualize_episode(network, grid, key, episode_num, gui):
    ...
    print(f"\nResult: {result} after {step_count} steps")
    print(f"Final score - P0: {int(obs_p0.owned_cells.sum())}, P1: {int(obs_p1.owned_cells.sum())}")
    
    return key, result


def render_state(state):
    """Simple text rendering of game state."""
    import numpy as np
    
    grid_size = state.armies.shape[0]
    
    # Convert to numpy for easier printing
    armies = np.array(state.armies)
    ownership_p0 = np.array(state.ownership[0])  # Player 0 ownership
    ownership_p1 = np.array(state.ownership[1])  # Player 1 ownership
    generals = np.array(state.generals)
    cities = np.array(state.cities)
    ...
    for r in range(grid_size):
        ...
        for c in range(grid_size):
            if mountains[r, c]:
                ...
                cell = f" G{int(armies[r, c]):2d} "
            elif generals[r, c] == 2:
                cell = f" g{int(armies[r, c]):2d} "
            elif cities[r, c]:
                if ownership_p0[r, c]:
                    cell = f" C{int(armies[r, c]):2d} "
                elif ownership_p1[r, c]:
                    cell = f" c{int(armies[r, c]):2d} "
                else:
                    cell = f" c{int(armies[r, c]):2d} "
            elif ownership_p0[r, c]:
                cell = f" {int(armies[r, c]):3d} "
            elif ownership_p1[r, c]:
                cell = f" {int(armies[r, c]):3d} "
            else:
                cell = f"  .  "
...
def main():
    ...
    print(f"JAX PPO - {num_envs} envs, {num_steps} steps/rollout")
    print(f"Device: {jax.devices()[0]}")
    print(f"Grid: 4x4 with composite rewards")
    print(f"Training for {num_iterations} iterations then visualizing 20 episodes")
    print()
    ...
    grid = grid.at[0, 0].set(1).at[grid_size-1, grid_size-1].set(2)  # Generals in corners
    grids = jnp.stack([grid] * num_envs)
    states = jax.vmap(game_jax.create_initial_state)(grids)
    
    print("\nWarming up...")
    for _ in range(3):
        states, _, key = rollout_step(states, network, grids, key)
    ...
    for iteration in range(num_iterations):
        ...
        for _ in range(num_steps):
            states, data, key = rollout_step(states, network, grids, key)
    ...
    # Visualize policy on 20 episodes
    print("\n" + "="*80)
    print("Visualizing policy on 20 episodes with GUI rendering...")
    print("="*80)
    print("Controls:")
    ...
    if total_episodes > 0:
        ...
        print(f"Wins:   {results['WIN']:2d} ({results['WIN']/total_episodes*100:.0f}%)")
        print(f"Losses: {results['LOSS']:2d} ({results['LOSS']/total_episodes*100:.0f}%)")
        print(f"Draws:  {results['DRAW']:2d} ({results['DRAW']/total_episodes*100:.0f}%)")
        print("="*80)
