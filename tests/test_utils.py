...
...B.
    """
    map = mapper.map_from_string(map)
...
def test_generate_map():
    ...
    for _ in range(5):
        map = mapper.map_from_generator(grid_size, mountain_density, city_density)
        assert mapper.validate_map(map)  # map has to be valid
        assert map.shape == (grid_size, grid_size)

    grid_size, mountain_density, town_density = 10, 0.2, 0.2
    for _ in range(5):
        map = mapper.map_from_generator(grid_size, mountain_density, town_density)
        assert mapper.validate_map(map)  # map has to be valid
        assert map.shape == (grid_size, grid_size)
