...
def _routable(obs, cell, walls, permissive):
    """Can a path plan cross this cell?

    Type 5 means "structure under fog" — which is a mountain OR a castle, and a
    castle is perfectly capturable. Refusing to route through 5 therefore treats
    every enemy castle as bedrock. That is fatal here: bots ring their own
    general with castles, so the general ends up enclosed by cells that merely
    *look* impassable and no attack path can ever be found.

    So: plan strictly first, and if nothing is reachable, allow fog-structures.
    Cells proven to be real mountains (we bounced off them) go into `walls`.
    """
    t = obs.type_grid[cell[0]][cell[1]]
    if t == 2:
        return False
    if t == 5:
        return permissive and cell not in walls
    return True


def _distances(obs, src, walls=frozenset(), permissive=False):
    """BFS distance from src to every reachable cell."""
    dist = {src: 0}
    queue = deque([src])
    while queue:
        cur = queue.popleft()
        ...
        for dr, dc in DIRECTIONS:
            ...
            if not (0 <= nxt[0] < obs.H and 0 <= nxt[1] < obs.W):
                continue
            if nxt in dist or not _routable(obs, nxt, walls, permissive):
                continue
            dist[nxt] = dist[cur] + 1
            queue.append(nxt)
    return dist
...
def _first_step(obs, src, targets, walls=frozenset(), permissive=False):
    """Direction of the first move on a shortest path from src to any target.

    Plain BFS — 484 cells on a competition board, so it is cheap enough to run
    every turn. Returns None if nothing is reachable. See _routable for why
    `permissive` exists.
    """
    ...
    while queue:
        ...
        for dr, dc in DIRECTIONS:
            ...
            if not (0 <= nxt[0] < obs.H and 0 <= nxt[1] < obs.W):
                continue
            if nxt in prev or not _routable(obs, nxt, walls, permissive):
                continue
            prev[nxt] = cur
            queue.append(nxt)
...
class Agent:
    ...
    def __init__(self, player_id, H, W):
        ...
        self.seen_enemy = set()
        # Fog-structures we have bounced off; they are real mountains.
        self.walls = set()
        self.fog_tries = {}
    ...
    def _attack_move(self, obs, hold, general):
        ...
        """
        # Deathtouch turns the endgame into a footrace: contact wins regardless
        # of army, so the only thing that matters is arriving. Send whichever
        # unit is FEWEST STEPS away, not the biggest one — the big stack is
        # usually deep in our own territory, and every extra step is a turn the
        # opponent can use to do the same thing to us.
        if self.enemy_general is not None and obs.turn >= DEATHTOUCH_TURN:
            rush = self._rush(obs, hold)
            if rush is not None:
                return rush
        ...
        d = self._route(obs, src, target)
        if d is None:
            return None
        return (0, src[0], src[1], d, 0)

    def _route(self, obs, src, targets):
        """First step toward `targets`, retrying through fog-structures."""
        d = _first_step(obs, src, targets, self.walls, False)
        if d is None:
            d = _first_step(obs, src, targets, self.walls, True)
        return d

    def _note_attempt(self, obs, move):
        """Learn which fog-structures are real mountains.

        Entering a mountain is an invalid move the engine silently turns into a
        pass, so a bad guess costs one turn. After two bounces off the same cell
        we stop routing through it — self-correcting, no map knowledge needed.
        """
        if move is None or move[0] != 0:
            return move
        _, r, c, d, _ = move
        nxt = (r + DIRECTIONS[d][0], c + DIRECTIONS[d][1])
        if not (0 <= nxt[0] < obs.H and 0 <= nxt[1] < obs.W):
            return move
        if obs.type_grid[nxt[0]][nxt[1]] == 5:
            self.fog_tries[nxt] = self.fog_tries.get(nxt, 0) + 1
            if self.fog_tries[nxt] >= 2:
                self.walls.add(nxt)
        return move

    def _rush(self, obs, hold):
        """Send the closest usable unit at the enemy general (deathtouch only).

        One BFS out from their general gives the step count for every cell at
        once, so picking the nearest of our stacks costs nothing extra.
        """
        dist = _distances(obs, self.enemy_general, self.walls, False)
        if not any(obs.owner_grid[r][c] == 1 and (r, c) in dist
                   for r in range(obs.H) for c in range(obs.W)):
            dist = _distances(obs, self.enemy_general, self.walls, True)
        best = None
        best_d = None
        for r in range(obs.H):
            for c in range(obs.W):
                if obs.owner_grid[r][c] != 1 or (r, c) in hold:
                    continue
                if obs.army_grid[r][c] < 2:       # one army always stays behind
                    continue
                d = dist.get((r, c))
        ...
        if best is None:
            return None
        # Step onto whichever neighbour is one closer to the target.
        for k, (dr, dc) in enumerate(DIRECTIONS):
            nxt = (best[0] + dr, best[1] + dc)
            if dist.get(nxt) == best_d - 1:
                return (0, best[0], best[1], k, 0)
        return None
    ...
    def act(self, obs):
        ...
        if attack is not None:
            return self._note_attempt(obs, attack)
