...
class Agent:
    ...
    def _attack_move(self, obs, hold, general):
        ...
        if target is None:
            # Scouting: the enemy's own land leads back to their general;
            # failing that, any fog at all is worth lifting.
            if self.seen_enemy:
                target = set(self.seen_enemy)
            else:
                target = self._fog_targets(obs, general)
