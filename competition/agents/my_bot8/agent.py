...
def _distances(obs, src):
    """BFS distance from src to every reachable passable cell."""
    dist = {src: 0}
    queue = deque([src])
    while queue:
        cur = queue.popleft()
        ...
        for dr, dc in DIRECTIONS:
            ...
            if not (0 <= nxt[0] < obs.H and 0 <= nxt[1] < obs.W):
                continue
            if nxt in dist or not _is_passable(obs.type_grid[nxt[0]][nxt[1]]):
                continue
            dist[nxt] = dist[cur] + 1
            queue.append(nxt)
    return dist
...
class Agent:
    ...
    def _rush(self, obs, hold):
        ...
        """
        dist = _distances(obs, self.enemy_general)
        best = None
        best_d = None
        for r in range(obs.H):
            for c in range(obs.W):
